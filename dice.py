...
# When player picks a die, dealer can only choose between remaining die
def dealer_choose_die(number):
    if number == 1:
...
def generate_my_result():
    ...
    return generate_dice_face_diagram(your_value), your_value, number


# calculates dealer's dice role result
def generate_dealer_result(num):
    number = dealer_choose_die(num)
    time.sleep(0.5)
